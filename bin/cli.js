...
cli
  ...
  .option(
    '-t, --timeout <timeout>',
    'The amount of time (in seconds) to wait for the response (Default 5s)',
    cli.INTEGER
  )
  ...
  .action(async (args, options, logger) => {
    if (!options.name || !options.uuid) return logger.error('Wrong number of option(s) for command refresh!');
