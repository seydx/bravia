...
class ServiceProtocol {
    ...
    if (this.methods.length > 0) {
      if (version) {
        return {
          ...
          methods: this.methods.filter((method) => method.version === version),
        };
      } else {
        return {
          protocol: this.protocol,
  ...
  async invoke(method, version = '1.0', data = [], headers = {}) {
    ...
    const service = await this.describe();
    ...
    if (!methodExist.length) {
      ...
      Object.assign(error, {
        title: `Unknown Service Method: "${method}"`,
        code: 'UNKNOWN',
        message: `Service Methods "${method}" not known!`,
        soap: {},
        payload: data,
        url: this.url,
      });
