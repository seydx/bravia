...
class Bravia {
  constructor(options) {
    ...
    this.timeout = options.timeout && options.timeout < 1000 ? options.timeout * 1000 : 5000;

    this.psk = options.psk;
    this.authWithPIN = options.pin;

    if (!this.psk && !this.authWithPIN) this.authWithPIN = true;
    ...
    if (this.psk) braviaDebug('Using PSK (%s) for authentication.', this.psk);

    if (this.authWithPIN) braviaDebug('Using PIN for authentication %O', this.credentials);
              ...
              if (!err) {
                try {
                  ...
                  if (device.serviceList) {
                    ...
                    let service = device.serviceList[0].service.find(
  ...
  async send(codes, delay) {
    ...
    for (const code of codes) {
      ...
      if (codes.length > 1) await TIMEOUT(delay);
    }
  ...
  async pair(user, pin, refresh) {
    try {
      ...
      if (response.headers['set-cookie']) {
        ...
        response.data &&
        response.data.error &&
        (response.data.error.includes(40005) ||
          response.data.error.includes('Display Is Turned off') ||
          response.data.error.includes('not power-on'))
      ) {
