...
class ServiceProtocol {
  ...
  async checkApiMethod(method, version, params) {
    ...
    if (apiMethod.length) {
      ...
      if (apiMethod.version !== version)
        this.debug(
          'Requested version (%s) for %s could not be found. Replacing it with v%s',
          version,
          method,
          apiMethod.version
        );
      ...
      this.debug(
        'Method options %O',
        apiMethod.methods.find((availableMethod) => availableMethod.method === method)
      );
  ...
  async invoke(method, version, params, turnOn) {
    ...
    if (method !== 'getVersions' && method !== 'getMethodTypes') {
      ...
      version = apiMethod ? apiMethod.version : version;
    }
